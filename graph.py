...
class MarketGraph(QtWidgets.QWidget):
    ...
    def update_orders(self, buy_orders, sell_orders):
        buy_x = [i for i in range(len(buy_orders))]
        sell_x = [i for i in range(len(sell_orders))]
        self.buy_orders_curve.setData(buy_x, buy_orders)
        self.sell_orders_curve.setData(sell_x, sell_orders)
