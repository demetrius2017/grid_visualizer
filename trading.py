...
class TradingSimulator:
    ...
    def set_grid_settings(self, settings):
        ...
        print(
            f"Grid settings updated: grid_size={self.grid_size}, volatility={self.volatility}"
        )

    def update(self):
        new_price = max(
            0, self.current_price + np.random.uniform(-self.volatility, self.volatility)
        )
    ...
    def update_display(self):
        ...
        executed_orders = [
            order for order in self.order_manager.get_order_history() if order.executed
        ]
    ...
    def update_report(self):
        ...
        hedge_metrics = (
            self.order_manager.get_hedge_metrics()
        )  # Получаем метрики хеджирования
    ...
    def update_balances(self):
        ...
        self.margin_history.append(
            self.order_manager.get_balance() - self.order_manager.get_free_margin()
        )
        if self.balance_history:  # Проверяем, есть ли вообще какие-либо данные
            self.graph.update_balance_graph(
                self.balance_history, self.free_margin_history, self.margin_history
            )

    def initialize_grid(self):
        if len(self.ema) > 0:
            print(
                f"Initializing grid with EMA: {self.ema[-1]}, Current Price: {self.current_price}"
            )
            self.order_manager.update_grid(
                self.ema[-1], self.current_price, self.prices
            )
        else:
        ...
        if self.graph.graphWidget.sceneBoundingRect().contains(pos):
            ...
            if modifiers == QtCore.Qt.ShiftModifier:
                self.current_price = max(
                    0,
                    cursor_position
                    + np.random.uniform(-self.volatility, self.volatility),
                )
